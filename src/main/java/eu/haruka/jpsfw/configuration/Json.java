...
public class Json {
     ...
     */
    public Json(File file, Class type) throws IOException {
     ...
     * @throws IOException if there is an error reading the file, or with the JSON syntax.
     */
    public Json(File file, TypeToken type) throws IOException {
        this();
        source = new String(Files.readAllBytes(Paths.get(file.toURI())), StandardCharsets.UTF_8);
        typeToken = type;
        object = load();
        if (object != null) {
     ...
     * @throws IOException if there is an error reading the file, or with the JSON syntax.
     */
    public Json(String string, Class type) throws IOException {
        this();
        source = string;
        this.type = type;
        object = load();
     ...
     *
     * @param object   The object to use as a JSON object.
     * @param compress Whether or not to disable pretty printing.
     */
    public Json(Object object, boolean compress) {
        this(compress);
        this.object = object;
        type = object.getClass();
        source = save();
    ...
    public Object load() throws IOException {
        try {
            if (typeToken != null) {
                return gson.fromJson(source, typeToken.getType());
            } else {
     ...
     */
    public static Object fromJson(String str, Class clazz) throws IOException {
        return new Json(str, clazz).getObject();
    }
